...
const employeeFeatures = [
  {
    description: "See assigned shifts and start times.",
    icon: CalendarDays,
    title: "View your schedule",
  },
  {
    description: "Keep your available hours up to date.",
    icon: Clock3,
    title: "Share availability",
  },
  {
    description: "Ask the team for help when plans change.",
    icon: UsersRound,
      ...
      <div className="rounded-[1.35rem] bg-white p-5 text-[#121824] shadow-xl sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 max-w-sm text-xl font-black tracking-tight">
              Everything you need for the week
            </h2>
          </div>
          <div className="hidden rounded-full bg-[#e8f3fb] px-3 py-1.5 text-xs font-bold text-[#0f62a3] sm:block">
            All in one place
          </div>
        </div>

        <div className="mt-5 grid grid-cols-3 gap-3">
          {employeeFeatures.map(({ description, icon: Icon, title }) => (
            <div
              className="rounded-2xl border border-[#dce4ec] bg-[#f7f9fb] p-4"
              key={title}
            >
              <div className="grid size-10 place-items-center rounded-xl bg-[#e8f3fb] text-[#0f62a3]">
                <Icon size={19} />
              </div>
              <h3 className="mt-4 text-sm font-black leading-tight">{title}</h3>
        ...
        <div className="mt-4 flex items-center gap-2 rounded-xl bg-[#eef4f8] px-4 py-3 text-xs font-semibold text-[#526071]">
          <LockKeyhole className="shrink-0 text-[#0f62a3]" size={16} />
          Your real schedule appears only after you sign in.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative flex min-h-screen flex-col lg:hidden">
      <div className="px-5 pb-8 pt-[max(1.25rem,env(safe-area-inset-top))]">
        <div
          className="relative h-[58px] w-[210px] overflow-hidden"
        ...
        <div className="mt-10 max-w-sm">
          <p className="text-[11px] font-extrabold tracking-[0.22em] text-[#78c4f3]">
            EMPLOYEE SCHEDULING
          </p>
      ...
      <section className="mt-auto rounded-t-[2rem] bg-[#f4f7fb] px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-7 text-[#121824] shadow-[0_-20px_55px_rgba(0,0,0,0.24)]">
        <div className="mx-auto w-full max-w-md">
          ...
          <p className="mt-5 text-xs font-extrabold tracking-[0.16em] text-[#0f62a3]">
            THE SCHEDULE
          </p>
          ...
          {isSignedInWithoutAccess ? (
            <>
              <div className="mt-5 rounded-2xl border border-[#e5b981] bg-[#fff8ed] p-4 text-sm leading-6 text-[#70420d]">
                ...
                <p className="mt-1 break-words text-[#70420d]/80">
                  You signed in as {signedInEmail}. Ask your manager to approve
            ...
            <>
              ...
              {hasAuthError ? (
                <div
                  className="mt-5 flex gap-3 rounded-2xl border border-[#e6b4b0] bg-[#fff2f1] p-4 text-sm leading-6 text-[#7c2420]"
                  role="alert"
                >
