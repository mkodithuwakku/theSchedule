...
For the September 16–30, 2026 schedule, once published, October 1–15 opens September 23, reminders start September 27, availability closes September 28, and the planned publication date is September 30. The manager must assign, review, and publish. The next October 16–31 window opens October 8 after the preceding schedule is published. September's published shifts and requests remain accessible while October is prepared.

The September 11 correction changed the current draft to September 16–30 while retaining its original period ID and all availability and invitation data. Its manager-selected availability deadline remains September 13 and release remains September 15. Old date-based IDs are stable references; do not rewrite them to match display dates.

For an explicitly authorized legacy draft repair, `scripts/correct-schedule-period.ts --store ID --period ID` is read-only by default (run with `node --env-file=.env --import tsx`). Review the reported change and current version before adding `--apply --expected-version N`. The script corrects only an unpublished 15–month-end draft, saves a manual workspace backup in the same serializable transaction, removes only draft shifts on the 15th, and preserves every other workspace field except the period label/start and an appended audit entry. It refuses changes that would orphan coverage or swap requests and aborts on concurrent saves. It never updates account or invitation tables. Preserve a full backup before running it; no reset, reseed, or schema migration is needed. Do not restore an old backup over subsequent employee submissions.
