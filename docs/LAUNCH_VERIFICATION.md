...
| Owner-only reset | Use the typed reset control before loading real operational data. Verify the owner is the sole active manager/person; every former employee is denied until newly invited. Check zero shifts/history and fresh suggested dates. | Workspace and membership counts, successful owner sign-in, rejected old employee access, rejected stale-tab write. Store configuration and pre-reset backup remain. |
...
2. Perform the owner-only reset as a deliberate UAT setup step; invite disposable test employees.
