...
As a fallback, the first successful workspace save on an Edmonton calendar day refreshes the protected copy when that day's cron backup has not yet run. This uses the same single row, so it adds resilience without adding storage growth.
