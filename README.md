...
Every successful workspace save refreshes the same protected row, so midweek changes are covered immediately without storage growth. The daily cron also guarantees a refresh on idle days. A `pre_reset` snapshot is held for the rest of its Edmonton calendar day so signing back in cannot immediately erase that recovery point.
